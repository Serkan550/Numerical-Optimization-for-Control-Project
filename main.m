...
method_Grad= "CentralFiniteDifferences"; % "ForwardFiniteDifferences" , "CentralFiniteDifferences" , "ImaginaryTrick(does not work!)"
method_LineSearch = "Backtracking"; % "Bisection", "Backtracking"
method_solver = "QuasiNewton-BFGS"; % "QuasiNewton-BFGS" , "fmincon"

% If u want to choose fmincon --> Recommended due to the optimization time
% (3-4 times faster than our basic BFGS algorithm :) )
if method_solver == "fmincon"
    optim_settings.Display = 'off';
    optim_settings.Algorithm = 'sqp';
end

% Assign the solver parameters
...
toc
%% -------------------- Animation  --------------------
fastscale = 5;   % animation speed: 1 = Normal ---> 3 = Fast ---> 5 = Really Fast
animate_ddrive_obs(Z_history, params, Ts, obs, goals', room, fastscale,goal_radius,robot_shape);
