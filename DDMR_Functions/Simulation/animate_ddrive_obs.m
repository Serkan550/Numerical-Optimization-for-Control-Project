    ...
    for k = 1:size(traj,2)
        ...
        for i = 1:length(obs)
            ...
            if strcmp(o{1}, 'circle') % The obstacle is Circular Type
                ...
                % Distance between COG of Robot and Center of Obstacle <=
                % Robot Safety Circle + Obstacle Radius
                if norm([xpos-cx, ypos-cy]) <= robot_radius + r, hit = true; break;end

            elseif strcmp(o{1}, 'rect') % The obstacle is Rectangular Type (which We did not use)
        ...
        % Take the current goal and its distance to COG position of Robot
        goal = goals(current_goal_idx,:);
        dist_to_goal = sqrt((xpos - goal(1))^2 + (ypos - goal(2))^2);

        if dist_to_goal <= goal_tolerance % Goal Tolerance defined or some precision error between COG pos of robot and Goal Point
            ...
            % Dynamical switch is actived --> New goal is defined
            current_goal_idx = current_goal_idx + 1;

            % If robot achieves all goals, the break the simulation
